...
class Ultrasonic extends EventEmitter {
  ...
  foundPeripheral (peripheral) {
    ...
    debug(`Found a peripheral: ${String(peripheral.advertisement.localName)} (match with ${this.options.name}? ${(String(peripheral.advertisement.localName).toUpperCase().trim() === this.options.name)})`)

    if (String(peripheral.advertisement.localName).toUpperCase().trim() === this.options.name) {
  ...
  decode (buf) {
    ...
    let windAngleApparent = this.degreesToRadians(buf.readUInt16LE(2))

    // Correct windAngleApparent so it's relative to bow
